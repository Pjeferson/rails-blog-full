...
RSpec.describe Post, type: :model do
  ...
  it 'should be valid with valide attributes' do
    post = Post.new(user_id: user.id, title: 'Post Title', body: 'body text')
    expect(post).to be_valid  
  end

  it 'should be valid without body' do
    post = Post.new(user_id: user.id, title: 'Post Title', body: '')
    expect(post).to be_valid  
  end

  it 'should not be valid without title' do
    post = Post.new(user_id: user.id, title: '', body: 'body text')
    expect(post).not_to be_valid  
  end

  it 'should not be valid with a title bigger than 50 chars' do
    post = Post.new(user_id: user.id, title: 'a' * 51, body: 'body text')
    expect(post).not_to be_valid  
  end

  it 'should not be valid with a body bigger than 240 chars' do
    post = Post.new(user_id: user.id, title: 'a' * 241, body: 'body text')
    expect(post).not_to be_valid  
  end
end
